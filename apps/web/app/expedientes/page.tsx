...
type Params = {q?:string;chamber?:string;status?:string;type?:string;period?:string;page?:string};
...
export default async function Expedients({searchParams}:{searchParams:Promise<Params>}) {
  ...
  const [listing, freshness, statuses]=await Promise.all([getApi<Listing>(path), getApi<{data:Freshness}>("/meta/freshness"), getApi<Catalog>("/catalogs/statuses")]);
