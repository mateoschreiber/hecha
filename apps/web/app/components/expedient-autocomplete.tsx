...
export function ExpedientAutocomplete({initial = ""}:{initial?:string}) {
  ...
  useEffect(() => { const controller=new AbortController(); if (!value) return void setItems([]); const timer=setTimeout(async()=>{ try { const r=await fetch(`/api/v1/expedients/suggest?q=${encodeURIComponent(value)}`,{signal:controller.signal}); const body=await r.json(); setItems(body.data ?? []); } catch {} }, 120); return ()=>{controller.abort();clearTimeout(timer)}; }, [value]);
